...
The random sample is of all numeric values, so the mean of this is easily calculated, equalling `r mean(test_df$rsamp)`. 

The mean of the logical vector works because `TRUE` is numerically 1 and `FALSE` is numerically 0, so being a random normally distributed samples, these average out to around 0.5 depending on each random sample, specifically `r mean(test_df$vec_logical)`. 
